...
-   rethink visualizations & add notes considering that they have to
    mean smth
-   consider dif number of trials, sessions, neurons, time, average, etc

### Success\~Mouse Bar Graph & Contingency Table (KEEP?)
...
#Plot
data.long %>% ggplot(aes(x=Names, y=Counts, fill=Category)) +
...
### Success \~ aggregate neuron firing

Currently what I've done here is I've made a pure heat map for every
session for the aggregate spikes across neurons and their respective
time bins. However, I don't really see any inference I can get with this
so it may get cut out. Especially since different sessions have
different \# trials and different \# neurons monitored

#keep? I NEEDA look at individual neurons

```{r}
session[[1]]$spks[[1]]

#Heat map function for aggregate spikes in a session
create_neuron_heatmap <- function(m) {
  matrix.aggregate = (matrix(nrow=nrow(session[[m]]$spks[[1]]), ncol=40))
  matrix.aggregate[is.na(matrix.aggregate)] <- 0
  for(i in 1:length(session[[m]]$spks)) {
    matrix.aggregate <- matrix.aggregate + session[[m]]$spks[[i]] #overlap the matrices to get a big aggregate one
  }
  #Modify data to make it readable into ggplot
  df.neurons <- matrix.aggregate %>% as.data.frame()
  df.neurons$Row <- 1:nrow(df.neurons)
  df.neurons <- pivot_longer(df.neurons, -Row, names_to = "Column", values_to = "Value")

  brain_areas <- c()
  num.row <- c()
  for(j in 1:length(session[[m]]$brain_area)){
    brain_areas <- c(brain_areas, rep(session[[m]]$brain_area[j], 40))
    num.row <- c(num.row, rep(j, 40))
  }

  #Arrange data frame in order of brain areas
  df.neurons <- cbind(df.neurons, brain_areas)
  df.neurons <- df.neurons %>% arrange(brain_areas)
  df.neurons$Row <- num.row
  df.neurons
  
  #plot heat map
  heat.map <- df.neurons %>% ggplot(aes(x= Column, y=Row, fill=Value )) +
    geom_tile() +
    scale_fill_gradient(low='white', high='blue') +
    ggtitle(paste("Aggregate spikes for session ", m))
  return(heat.map)
}
...
#for(i in 1:18){
#  create_neuron_heatmap(i)
#}

#HTML scuffs this function when you put too many inputs
grid.arrange(create_neuron_heatmap(1), create_neuron_heatmap(2), ncol=2)
grid.arrange(create_neuron_heatmap(3), create_neuron_heatmap(4), ncol=2)
grid.arrange(create_neuron_heatmap(5), create_neuron_heatmap(6), ncol=2)
grid.arrange(create_neuron_heatmap(7), create_neuron_heatmap(8), ncol=2)
grid.arrange(create_neuron_heatmap(9), create_neuron_heatmap(10), ncol=2)
grid.arrange(create_neuron_heatmap(11), create_neuron_heatmap(12), ncol=2)
grid.arrange(create_neuron_heatmap(13), create_neuron_heatmap(14), ncol=2)
grid.arrange(create_neuron_heatmap(15), create_neuron_heatmap(16), ncol=2)
grid.arrange(create_neuron_heatmap(17), create_neuron_heatmap(18), ncol=2)

#Issue is it still can't tell me that much data nor is it split by successes and failures
#at best maybe compare them with more successful sessions but even then, it's hard to fully tell which neuon is which SINCE THEY'RE DIFFERNET
```

### Success \~ neuron firing in specific brain areas
...
#EDIT change to average
#I averaged it out by brain area but do i also need to average it by trial? Probably not?
#IF we really wanna compare success and failure though, we should average it out by number of success trials and number of failure trials. That way, even though the neurons are unevenly distributed, it doesn't matter since we're comparing the success & Failures
#With them averagd by trial, we'll be able to see smth about if total spikes matters &&&& as it so happens, it generally does not. So we know the number of brain spikes doesn't matter. whether brain areas matter, i think it's also a bust if the distribution looks generally the same when we average by brain area

#IF we average by both and there's the same distribution + same scaling, then yeah it's a bust

##YEAH this is dead... it's probably specific neurons 
...
create_prop_contrast_bar <- function(n, abs){
  ...
  plot.bar <- diff %>% ggplot(aes(x=Contrast,y=props)) +
    ...
    labs(title = paste("Session ", 1, ",", session[[n]]$mouse_name))
  return(plot.bar)
}
...
#EVENTUALLY, I needa fit it better probably with an appendix
#I may also need to visualize individual neurons
